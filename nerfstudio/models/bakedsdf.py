...
class BakedSDFFactoModel(VolSDFModel):
    ...
    def sample_and_forward_field(self, ray_bundle: RayBundle):
        ...
        delta_density = ray_samples.deltas * field_outputs[FieldHeadNames.DENSITY]
        field_outputs[FieldHeadNames.ALPHA] = 1 - torch.exp(-delta_density)

        if self.config.background_model != "none":
            field_outputs = self.forward_background_field_and_merge(ray_samples, field_outputs)

        weights, transmittance = ray_samples.get_weights_and_transmittance_from_alphas(field_outputs[FieldHeadNames.ALPHA])
